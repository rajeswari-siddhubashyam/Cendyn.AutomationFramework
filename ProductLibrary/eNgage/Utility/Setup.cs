...
namespace eNgage.Utility
{
    public class Setup : BaseUtility.Utility.Setup
    {
        ...
        public static void InitializeSetup(string testPlanId, string projectName = null)
        {
            ...
            Setup.LocalMachineExecution = 0;// Convert.ToInt32(LegacyTestData.LambdaExecution);
            Setup.LT_USERNAME = "sborade";// LegacyTestData.LambdaUserName;
            Setup.LT_ACCESS_KEY = "KjfLHzAi54WQDIWauYpwEE8wx9NPdAmyldXzx3h3apEqaQD9jw";// LegacyTestData.LambdaAccessKey;
            Setup.build = Environment.GetEnvironmentVariable("LT_BUILD") == null ? "eNgage_LambdaTestExecution_" + DateTime.Now.ToString("MM") + DateTime.Now.ToString("dd") + DateTime.Now.ToString("yyyy") : Environment.GetEnvironmentVariable("LT_BUILD");
            Setup.seleniumUri = LegacyTestData.LambdaHub;


            SetupTestPlan(eProduct.eNgage, clientName, testDataType, caseType, projectName);
        ...
        public static void SetupConnectionString()
        {
            ...
            ConnectionString3 = ConfigurationManager.ConnectionStrings["eNgagePromptDBConn"].ConnectionString;
        }
