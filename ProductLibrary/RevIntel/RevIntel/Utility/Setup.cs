...
namespace RevIntel.Utility
{
    public class Setup : BaseUtility.Utility.Setup
    {
        ...
        public static void InitializeSetup(string testPlanId, Enum clientName, Enum testDataType, Enum caseType, string projectName = null)
        {
            ...
            Setup.build = Environment.GetEnvironmentVariable("LT_BUILD") == null ? "RevIntel_LambdaTestExecution_" + DateTime.Now.ToString("MM") + DateTime.Now.ToString("dd") + DateTime.Now.ToString("yyyy") : Environment.GetEnvironmentVariable("LT_BUILD");
            Setup.LocalMachineExecution = 1;
            SetupTestPlan(eProduct.RevIntel, clientName, testDataType, caseType, projectName);
