    ...
    public class Setup : BaseUtility.Utility.Setup
    {
        ...
        public static void InitializeSetup(string testPlanId, Enum clientName, Enum testDataType, Enum caseType, string projectName = null)
        {
            ...
            //Initialize the test case set up.
                ...
                if(projectName == "ePlanner")
                {
                    Setup.build = Environment.GetEnvironmentVariable("LT_BUILD") == null ? "ePlanner_LambdaTestExecution_" + DateTime.Now.ToString("MM") + DateTime.Now.ToString("dd") + DateTime.Now.ToString("yyyy") : Environment.GetEnvironmentVariable("LT_BUILD");
                    SetupTestPlan(eProduct.ePlanner, clientName, testDataType, caseType, projectName);
                }   
                    
                else if(projectName == "eMenusLite")
                {
                    Setup.build = Environment.GetEnvironmentVariable("LT_BUILD") == null ? "eMenusLite_LambdaTestExecution_" + DateTime.Now.ToString("MM") + DateTime.Now.ToString("dd") + DateTime.Now.ToString("yyyy") : Environment.GetEnvironmentVariable("LT_BUILD");
                    SetupTestPlan(eProduct.eMenusLite, clientName, testDataType, caseType, projectName);
            }
                    
                else if (projectName == "eMenus")
                {
                    Setup.build = Environment.GetEnvironmentVariable("LT_BUILD") == null ? "eMenus_LambdaTestExecution_" + DateTime.Now.ToString("MM") + DateTime.Now.ToString("dd") + DateTime.Now.ToString("yyyy") : Environment.GetEnvironmentVariable("LT_BUILD");
                    SetupTestPlan(eProduct.eMenus, clientName, testDataType, caseType, projectName);
                }
