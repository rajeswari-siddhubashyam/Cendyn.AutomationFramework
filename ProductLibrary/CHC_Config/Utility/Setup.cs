...
namespace CHC_Config.Utility
{
    public class Setup : BaseUtility.Utility.Setup
    {
        ...
        public static void InitializeSetup(string testPlanId, Enum clientName, Enum testDataType, Enum caseType, string projectName = null)
        {
            ...
            Setup.build = Environment.GetEnvironmentVariable("LT_BUILD") == null ? "CHC_Config_LambdaTestExecution_" + DateTime.Now.ToString("MM") + DateTime.Now.ToString("dd") + DateTime.Now.ToString("yyyy") : Environment.GetEnvironmentVariable("LT_BUILD");

            //Initialize the test case set up.
            SetupTestPlan(eProduct.CHC_Config, clientName, testDataType, caseType, projectName);

            //Set Connection String
            SetupConnectionString(Constants.clientEnv.CHC_ConfigQA);
